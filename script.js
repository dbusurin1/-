document.addEventListener('DOMContentLoaded', function() {
    ...
    const magicBallModal = document.getElementById('magic-ball-modal');
    const spinButton = document.getElementById('spin-button');
    const spinAgainButton = document.getElementById('spin-again-button');
    const closeButton = document.getElementById('close-button');
    const closeMagicBallButton = document.getElementById('close-magic-ball-button');
    ...
    const magicBallAnswer = document.getElementById('magic-ball-answer');
    ...
    // Magic Ball functions
    function shakeMagicBall() {
        if (isShaking) return;
        
        isShaking = true;
        magicBall.style.animation = 'shake 0.5s';
        magicBallAnswer.textContent = '';
        
        setTimeout(() => {
            magicBall.style.animation = '';
            const randomAnswer = MAGIC_BALL_ANSWERS[Math.floor(Math.random() * MAGIC_BALL_ANSWERS.length)];
            magicBallAnswer.textContent = randomAnswer;
            isShaking = false;
        }, 500);
    }
    ...
    decideButton.addEventListener('click', function() {
        magicBallModal.style.display = 'flex';
        magicBallAnswer.textContent = '8';
    });
    
    magicBall.addEventListener('click', shakeMagicBall);
    
    closeMagicBallButton.addEventListener('click', function() {
        magicBallModal.style.display = 'none';
    });
