...
const CONFIG = {
    ...
    TARGET_REACH: 66.9, // Целевой охват
    CHANNELS: {
        video: {
            maxPercent: 100,
            maxReach: 25,
...
function calculateChannelReach(budget, channel) {
    ...
    const maxBudget = CONFIG.FIXED_BUDGET * (state.channelPercents[channel] / 100);
    
    if (maxBudget === 0) return 0;
    
    const normalizedBudget = budget / maxBudget;
    let reach = 0;
...
function calculateTotalReach(budget = null) {
    ...
    // Рассчитываем бюджеты и охваты для каждого канала
    Object.keys(state.channelPercents).forEach(channel => {
        budgets[channel] = currentBudget * (state.channelPercents[channel] / 100);
        reaches[channel] = calculateChannelReach(budgets[channel], channel);
    });
